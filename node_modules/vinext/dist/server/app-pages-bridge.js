...
async function renderPagesFallback(options, dependencies) {
	...
	if (pagesPathname.startsWith("/api/") || pagesPathname === "/api") {
		...
		const pagesApiResponse = await pagesEntry.handleApiRoute(pagesRequest, pagesUrl, void 0, getRequestExecutionContext()?.trustedRevalidateOrigin ?? new URL(pagesRequest.url).origin);
